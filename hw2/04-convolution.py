...
GKernel = numpy.array([[0,0.25,0], 
						[0.25,1,0.25], 
						[0,0.25,0]])

RKernel = numpy.array([[0.25, 0.50, 0.25], 
						[0.5,1,0.5], 
						[0.25,0.5,0.25]])
BKernel = numpy.array([[0.25, 0.50, 0.25], 
						[0.5,1,0.5], 
						[0.25,0.5,0.25]])
...
numpyOutput[:,:,0] = cv2.filter2D(src=numpyOutput[:,:,0], kernel=BKernel, ddepth=-1, borderType=cv2.BORDER_DEFAULT)
numpyOutput[:,:,2] = cv2.filter2D(src=numpyOutput[:,:,2], kernel=RKernel, ddepth=-1, borderType=cv2.BORDER_DEFAULT)
numpyOutput[:,:,1] = cv2.filter2D(src=numpyOutput[:,:,1], kernel=GKernel, ddepth=-1, borderType=cv2.BORDER_DEFAULT)
# cv2.imshow('output', numpyOutput)
# cv2.waitKey(0)
