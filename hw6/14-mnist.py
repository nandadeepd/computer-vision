...
for tensorInput, tensorTarget in objectDataset:
		...
		# iterating through the batch of 64 where i is the index of training example - used to pass it to write_image()
		for i, (pred, true) in enumerate(zip(variableEstimate.data.max(dim=1, keepdim=False)[1], variableTarget.data)):
			if true != pred:
				write_image(tensorInput[i], true, pred)
